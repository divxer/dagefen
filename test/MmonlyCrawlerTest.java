...
/**
 * User: divxer Date: 12-11-5 Time: 下午11:58
 */
public class MmonlyCrawlerTest {
  public static void main(String[] args) {
    try {
      Document doc = Jsoup.connect("http://www.mmonly.com").timeout(10000).get();

      if (doc == null) {
        Logger.error("http://www.mmonly.com is empty!");
        return;
      }
      ...
      System.out.println("list Page size= " + listPage.size());
  ...
  private static void processChannel(String channelUrl) {
    try {
      ...
      while (pageMatcher.find()) {
        ...
        System.out.println("total page for " + channelUrl + ": " + totalPages);
  ...
  private static void processPage(String pageUrl) {
    try {
      ...
      Elements articleList = doc.select("html body div.wrap div.w650 div.imgList ul li");
        ...
        System.out.println("page link: " + pageUrl + " size:" + articleList.size());
  ...
  private static void processArticle(String articleUrl) {
    try {
      ...
      if (titleElement == null) {
          System.out.println("page link: " + articleUrl + " is null");
          return;
      }

      String title = titleElement.text();

        System.out.println("picture title: " + title);

      Element descriptionElement =
          doc.select("html body#body div.wrap div.arcOther div.arcDES").first();
